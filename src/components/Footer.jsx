    ...
    <footer className="bg-[#131514] text-white">
      <div className="border-b border-white/15">
        <div className="mx-auto flex max-w-[1320px] flex-col gap-7 px-5 py-12 sm:flex-row sm:items-center sm:justify-between lg:px-8">
          ...
          <Link to="/#reservation" className="inline-flex w-fit items-center gap-3 bg-[#F1EEE8] px-6 py-4 text-[12px] font-semibold text-[#131514] transition hover:bg-white">Get my ride quote <ArrowUpRight className="h-4 w-4" /></Link>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[1320px] px-5 py-16 lg:px-8">
        <div className="grid gap-12 border-b border-white/15 pb-14 md:grid-cols-2 lg:grid-cols-12">
          ...
          <div className="lg:col-span-2">
            ...
            <nav className="mt-5 flex flex-col gap-3 text-xs text-white/60">
              ...
              <Link to="/#fleet" className="hover:text-white transition">First-class fleet</Link>
              <Link to="/#ride-options" className="hover:text-white transition">Ride options</Link>
              <Link to="/#experience" className="hover:text-white transition">How it works</Link>
              <Link to="/#testimonials" className="hover:text-white transition">Reviews</Link>
          ...
          <div className="lg:col-span-2">
            ...
            <nav className="mt-5 flex flex-col gap-2 text-xs text-white/60">
              {locationsData.map((location) => (
                <Link key={location.name} to={`/service-areas/${location.name.toLowerCase()}`} className="hover:text-white transition">{location.name}</Link>
              ))}
            </nav>
          </div>
          <div className="lg:col-span-3">
            <h3 className="text-[9px] font-bold uppercase tracking-[0.18em] text-gold">Direct dispatch</h3>
            <a href={`tel:${companyInfo.phoneRaw}`} className="mt-5 flex items-center gap-2 font-serif text-2xl text-white hover:text-gold transition"><Phone className="h-4 w-4 text-gold" />{companyInfo.phone}</a>
        ...
        <div className="flex flex-col gap-5 pt-8 text-[9px] uppercase tracking-[0.12em] text-white/35 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-6"><a href="#" className="hover:text-white transition">Privacy</a><a href="#" className="hover:text-white transition">Terms</a><button type="button" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} aria-label="Back to top" className="flex h-9 w-9 items-center justify-center border border-white/20 text-white hover:border-gold hover:text-gold transition"><ArrowUp className="h-4 w-4" /></button></div>
        </div>
      </div>
      <GhlChatWidget />
    </footer>
