      ...
      <div className="mx-auto max-w-[1320px] px-5 lg:px-8">
        <div className="grid gap-10 lg:grid-cols-12">
          ...
          <motion.div
            ...
            whileInView="show"
            viewport={viewportConfig}
            className="border-t border-black/20 lg:col-span-7 lg:col-start-6"
          >
            {locationsData.map((location) => (
              <motion.div key={location.name} variants={fadeUp}>
                <Link
                  id={`area-${location.name.toLowerCase()}`}
                  to={`/service-areas/${location.name.toLowerCase()}`}
                  className="group grid grid-cols-[1fr_auto] items-center gap-4 border-b border-black/20 py-6 transition-all duration-200 hover:pl-2 hover:bg-black/[0.015]"
                >
                  <div>
                    <h3 className="font-serif text-3xl font-medium tracking-[-0.025em] text-[#131514]">{location.name}</h3>
